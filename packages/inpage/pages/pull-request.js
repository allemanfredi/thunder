...
const PullRequest = {
  ...
  injectElements(_web3, _request) {
    ...
    document.querySelector('#new_pull_request > div > div.col-9 > div > div > tab-container')
  ...
  async handleSubmit(_event, _web3,  _request) {
    ...
    const whoPerformPullRequest =  document.querySelector("#new_pull_request > div > div.col-9 > div > span > a > img").alt
    const pullRequestTextBody = document.querySelector('#pull_request_body').value
    ...
    const whoPerformPullRequestEthAddress = extrapolateEthAddressFromBio(info.bio)
    if (!whoPerformPullRequestEthAddress) {
      console.info('Please paste your ETH address in your bio preceded by Ξ or check that is correct')
      return
    }
