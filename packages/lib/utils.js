...
const extrapolateIssueNumberFromText = _text => {
  ...
  if (words[indexPresentSpecialKeyword + 1].match('#[1-9][1-9]*')) {
    return words[indexPresentSpecialKeyword + 1].substr(1, words[indexPresentSpecialKeyword + 1].length)
  }
}

const getIssueNumberFromIssue = _issue => {
  return _issue.attributes[0].value.substr(_issue.attributes[0].value.length - 1, 1)
}
